...
window.NIRMAAN_MAP = {
  ...
  setupFilterControls() {
    ...
    const gatiShaktiBtn = document.getElementById('toggle-gati-shakti-btn');
    if (gatiShaktiBtn) {
      gatiShaktiBtn.addEventListener('click', () => {
        this.isGatiShaktiMode = !this.isGatiShaktiMode;
        if (this.isGatiShaktiMode) {
          gatiShaktiBtn.style.background = 'linear-gradient(135deg, #a855f7, #6b21a8)';
          gatiShaktiBtn.innerHTML = '<i class="fa-solid fa-network-wired"></i> NMP Layer Active';
          if (window.showGlobalToast) window.showGlobalToast('🌐 PM Gati Shakti NMP Multi-Modal Layer & Spatial Conflict Scanner Activated', 'success');
          this.renderGatiShaktiLayers();
        } else {
          gatiShaktiBtn.style.background = 'linear-gradient(135deg, #8b5cf6, #4c1d95)';
          gatiShaktiBtn.innerHTML = '<i class="fa-solid fa-network-wired"></i> PM Gati Shakti NMP';
          if (this.gatiShaktiGroup) this.gatiShaktiGroup.clearLayers();
          if (window.showGlobalToast) window.showGlobalToast('PM Gati Shakti NMP Layer Hidden', 'info');
        }
      });
    }
  },
  ...
  renderGatiShaktiLayers() {
    if (!this.mapInstance) return;
    if (!this.gatiShaktiGroup) {
      this.gatiShaktiGroup = L.layerGroup().addTo(this.mapInstance);
    }
    this.gatiShaktiGroup.clearLayers();

    // 1. NHAI Expressway Network Corridor (Cyan Dashed)
    const nhaiLine = L.polyline([
    ...
    nhaiLine.bindTooltip("🛣️ NHAI National Expressway Corridor (PM Gati Shakti NMP)", { sticky: true });
    this.gatiShaktiGroup.addLayer(nhaiLine);

    // 2. Dedicated Freight Corridor DFC (Amber Solid)
    const dfcLine = L.polyline([
      [28.6139, 77.2090], // Delhi
    ...
    dfcLine.bindTooltip("🚆 Eastern Dedicated Freight Corridor (DFC)", { sticky: true });
    this.gatiShaktiGroup.addLayer(dfcLine);

    // 3. PowerGrid HVDC Transmission Line (Purple Solid)
    ...
    powerLine.bindTooltip("⚡ PowerGrid 400kV HVDC Transmission Grid", { sticky: true });
    this.gatiShaktiGroup.addLayer(powerLine);

    // 4. Interactive PM Gati Shakti Spatial Conflict Pins
    // Conflict 1: Palghar, MH [19.6967, 72.7699]
    const conflictIcon1 = L.divIcon({
      className: 'custom-gis-pin-wrapper',
      html: `
        <div class="gis-marker-pin" style="background: linear-gradient(135deg, #a855f7, #6b21a8); color: white; border: 2px solid #e9d5ff; box-shadow: 0 0 20px rgba(168, 85, 247, 0.9);">
          <div class="marker-pulse-ring" style="background: rgba(168, 85, 247, 0.6); animation: drone-radar-pulse 1.2s infinite ease-out;"></div>
          <i class="fa-solid fa-network-wired" style="font-size: 0.85rem; z-index: 2;"></i>
        </div>
      `,
      iconSize: [38, 38],
      iconAnchor: [19, 19],
    ...
    const conflict1 = L.marker([19.6967, 72.7699], { icon: conflictIcon1 });
    conflict1.bindPopup(`
      <div style="background: #0f172a; color: white; padding: 10px; border-radius: 10px; border: 1px solid #c084fc; max-width: 280px;">
        <div style="font-size: 0.68rem; font-weight: 800; color: #c084fc; text-transform: uppercase;">PM Gati Shakti Spatial Conflict</div>
        <h4 style="font-size: 0.88rem; font-weight: 800; color: white; margin: 4px 0;">CONFLICT-NMP-801: Rail Viaduct ✕ NH-61 Crossing</h4>
        <div style="font-size: 0.72rem; color: #cbd5e1; margin-bottom: 8px;">Pier #142 overlaps with NHAI Median ROW in Palghar.</div>
        <div style="display: flex; justify-content: space-between; font-size: 0.72rem; background: rgba(255,255,255,0.05); padding: 4px 8px; border-radius: 6px; margin-bottom: 8px;">
          <span>Est. Delay: <strong style="color: #f87171;">14 Months</strong></span>
          <span>Overrun: <strong style="color: #f87171;">₹180 Cr</strong></span>
        </div>
        <button onclick="document.querySelectorAll('.btn-issue-npg-directive')[0]?.click();" style="width: 100%; background: linear-gradient(135deg, #8b5cf6, #6d28d9); border: none; color: white; padding: 6px; border-radius: 6px; font-weight: 800; font-size: 0.73rem; cursor: pointer;">
          <i class="fa-solid fa-file-signature"></i> Resolve via NPG Directive
        </button>
      </div>
    `, { className: 'nirmaan-custom-popup' });
    this.gatiShaktiGroup.addLayer(conflict1);

    // Conflict 2: Haridwar, UK [29.9457, 78.1642]
    const conflictIcon2 = L.divIcon({
      className: 'custom-gis-pin-wrapper',
      html: `
        <div class="gis-marker-pin" style="background: linear-gradient(135deg, #a855f7, #6b21a8); color: white; border: 2px solid #e9d5ff; box-shadow: 0 0 20px rgba(168, 85, 247, 0.9);">
          <div class="marker-pulse-ring" style="background: rgba(168, 85, 247, 0.6); animation: drone-radar-pulse 1.2s infinite ease-out;"></div>
    ...
    const conflict2 = L.marker([29.9457, 78.1642], { icon: conflictIcon2 });
    conflict2.bindPopup(`
      <div style="background: #0f172a; color: white; padding: 10px; border-radius: 10px; border: 1px solid #c084fc; max-width: 280px;">
        <div style="font-size: 0.68rem; font-weight: 800; color: #c084fc; text-transform: uppercase;">PM Gati Shakti Spatial Conflict</div>
        <h4 style="font-size: 0.88rem; font-weight: 800; color: white; margin: 4px 0;">CONFLICT-NMP-802: Delhi Expressway ✕ PowerGrid HVDC</h4>
        <div style="font-size: 0.72rem; color: #cbd5e1; margin-bottom: 8px;">Tower height clearance deadlock in Haridwar section.</div>
        <div style="display: flex; justify-content: space-between; font-size: 0.72rem; background: rgba(255,255,255,0.05); padding: 4px 8px; border-radius: 6px; margin-bottom: 8px;">
          <span>Est. Delay: <strong style="color: #fbbf24;">8 Months</strong></span>
          <span>Overrun: <strong style="color: #fbbf24;">₹65 Cr</strong></span>
        </div>
        <button onclick="document.querySelectorAll('.btn-issue-npg-directive')[1]?.click();" style="width: 100%; background: linear-gradient(135deg, #8b5cf6, #6d28d9); border: none; color: white; padding: 6px; border-radius: 6px; font-weight: 800; font-size: 0.73rem; cursor: pointer;">
          <i class="fa-solid fa-file-signature"></i> Resolve via NPG Directive
        </button>
      </div>
    `, { className: 'nirmaan-custom-popup' });
    this.gatiShaktiGroup.addLayer(conflict2);
  }
};
